...
return {
	...
	-----------------------------------------------------------------------------
	{
		...
		opts = function()
			local icons = require('jaime.config').icons
			local get_color = require('jaime.lib.color').get_color
			local fg = function(...) return { fg = get_color('fg', ...) } end

			local function filepath()
				local fpath = require('jaime.lib.badge').filepath(0, 3, 5)
			...
			local theme = {
				normal = {
					...
					y = {
						fg = active.fg,
						bg = require('jaime.lib.color').brightness_modifier(active.bg, -20),
					},
					z = {
						fg = active.fg,
						bg = require('jaime.lib.color').brightness_modifier(active.bg, 63),
					},
				},
				inactive = {
			...
			return {
				...
				sections = {
					...
					lualine_b = {
						{
							function() return require('jaime.lib.badge').icon() end,
							padding = { left = 1, right = 0 },
						},
						{
							filepath,
					...
					lualine_c = {
						...
						-- Whitespace trails
						{
							function() return require('jaime.lib.badge').trails('␣') end,
							cond = is_file_window,
							padding = { left = 1, right = 0 },
					...
					lualine_y = {
						{
							function() return require('jaime.lib.badge').filemedia('  ') end,
