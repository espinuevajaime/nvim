...
function M.get()
	...
	local format = function()
		require('jaime.plugins.lsp.format').format({ force = true })
	end
	...
	M._keys =  {
		...
		{ 'K', function()
			...
			local winid = require('jaime.lib.utils').has('nvim-ufo')
...
function M.resolve(buffer)
	...
	local opts = require('jaime.lib.utils').opts('nvim-lspconfig')
	local clients
	if vim.lsp.get_clients ~= nil then
		clients = vim.lsp.get_clients({ bufnr = buffer })
	else
