	...
	-----------------------------------------------------------------------------
	{
		...
		dependencies = {
			...
			'williamboman/mason-lspconfig.nvim',
			{
				...
				cond = function()
					return require('jaime.lib.utils').has('nvim-cmp')
				end,
			},
		},
		---@class PluginLspOpts
		...
		config = function(_, opts)
			if require('jaime.lib.utils').has('neoconf.nvim') then
				local plugin = require('lazy.core.config').spec.plugins['neoconf.nvim']
				require('neoconf').setup(require('lazy.core.plugin').values(plugin, 'opts', false))
			end
			-- Setup autoformat
			require('jaime.plugins.lsp.format').setup(opts)
			-- Setup formatting, keymaps and highlights.
			local lsp_on_attach = require('jaime.lib.utils').on_attach
			---@param client lsp.Client
			---@param buffer integer
			lsp_on_attach(function(client, buffer)
				require('jaime.plugins.lsp.keymaps').on_attach(client, buffer)
				require('jaime.plugins.lsp.highlight').on_attach(client, buffer)
			...
			for type, icon in pairs(require('jaime.config').icons.diagnostics) do
			...
			then
				opts.diagnostics.virtual_text.prefix = vim.fn.has('nvim-0.10') == 0
					...
					or function(diagnostic)
						local icons = require('jaime.config').icons.diagnostics
