...
-- This file is automatically loaded by jaime.config.init

local Util = require('jaime.lib.utils')
local map = vim.keymap.set

local function augroup(name)
	return vim.api.nvim_create_augroup('jaime_' .. name, {})
end

-- Elite-mode: Arrow-keys resize window
if vim.g.jaime_elite_mode then
...
map('n', ']s', function()
	require('jaime.lib.edit').whitespace_jump(1)
...
map(
	'x',
	'<C-r>',
	":<C-u>%s/\\V<C-R>=v:lua.require'jaime.lib.edit'.get_visual_selection()<CR>"
		.. '//gc<Left><Left><Left>',
...
map('n', '<leader>uf', require('jaime.plugins.lsp.format').toggle, { desc = 'Toggle format on Save' })
...
map('n', '<LocalLeader>c', function()
	require('jaime.lib.contextmenu').show()
...
-- Ultimatus Quitos
if vim.F.if_nil(vim.g.jaime_window_q_mapping, true) then
...
map('n', '<Leader>q', function()
	require('jaime.lib.edit').toggle_list('quickfix')
...
map('n', '<Leader>a', function()
	...
	require('jaime.lib.edit').toggle_list('loclist')
